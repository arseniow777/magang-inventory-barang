...
//   await bot.sendMessage(chatId, `Halo, *${name}*! Pilih menu yang tersedia:`, {
//     parse_mode: "Markdown",
...
//             `${API_URL}/api/v1/bot/reports/${report.id}/download`,
//           );
...
//           await bot.sendMessage(
//             chatId,
//             "Request tidak valid untuk dikembalikan.",
//           );
//         } else {
...
//         await bot.sendMessage(
//           chatId,
//           `⚠️ *Konfirmasi Disconnect*\n\nApakah Anda yakin ingin memutus koneksi akun *${user.username}* dari Telegram?\n\nSetelah disconnect, Anda perlu generate link baru dari website untuk terhubung kembali.`,
//           {
//             parse_mode: "Markdown",
//             reply_markup: {
//               inline_keyboard: [
...
//                   {
//                     text: "✅ Ya, Disconnect",
//                     callback_data: "disconnect_confirm",
//                   },
//                   { text: "❌ Batalkan", callback_data: "disconnect_cancel" },
//                 ],
//               ],
//             },
//           },
...
//         await bot.sendMessage(
//           chatId,
//           `Akun *${user.username}* berhasil di-disconnect dari Telegram.`,
//           {
//             parse_mode: "Markdown",
//           },
//         );
...
//         await bot.sendMessage(chatId, "Disconnect dibatalkan.", {
//           reply_markup: getMenu(),
//         });
//       }
...
//         await bot.sendMessage(
//           chatId,
//           "Format salah. Gunakan: `username|passwordbaru`",
//           { parse_mode: "Markdown" },
//         );
//         return;
//       }
...
//           await bot.sendMessage(
//             chatId,
//             "Username ini sudah terdaftar dengan Telegram lain.",
//           );
//           return;
//         }
...
if (bot) {
    ...
    if (param) {
      try {
        ...
        if (decoded.startsWith("link:")) {
          ...
          const alreadyUsed = await prisma.users.findFirst({
            where: { telegram_id, id: { not: user.id } },
          });
  ...
  bot.on("callback_query", async (query) => {
    ...
    try {
      ...
      if (data === "menu_reports") {
        const report = await prisma.officialReports.findFirst({
          where: { request: { pic_id: user.id }, is_approved: true },
          orderBy: { issued_date: "desc" },
        });
      ...
      if (data.startsWith("return_confirm:")) {
        ...
        const request = await prisma.requests.findUnique({
          ...
          include: {
            request_items: {
              include: { unit: { include: { location: true } } },
            },
          },
        });
      ...
      if (data === "menu_reset_password") {
        awaitingPassword.set(telegram_id, true);
        await bot.sendMessage(
          chatId,
  ...
  bot.on("message", async (msg) => {
    ...
    // Handle awaiting password input
    if (awaitingPassword.get(telegram_id)) {
      awaitingPassword.delete(telegram_id);
      ...
      try {
        ...
        const new_password_hash = await bcrypt.hash(text, 10);
        const reset = await prisma.passwordResets.create({
