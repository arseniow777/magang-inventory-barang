...
export function Pattern({
  ...
  const getFileIcon = (file: File | FileMetadata) => {
    ...
    if (type.startsWith("video/")) return <IconVideo className="size-4" />;
    if (type.startsWith("audio/")) return <IconHeadphones className="size-4" />;
    if (type.includes("pdf")) return <IconFile className="size-4" />;
    if (type.includes("word") || type.includes("doc"))
      return <IconFile className="size-4" />;
    if (type.includes("excel") || type.includes("sheet"))
      return <IconTable className="size-4" />;
